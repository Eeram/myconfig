...
log = None
GITHUB_SLUG = 'mathieumaree/alfred-bluetooth-workflow'

def _read_devices():
    proc = subprocess.Popen(['./lib/blueutil', '--paired', '--format=JSON'], stdout=subprocess.PIPE)
    ...
    for device in devices_raw:
        if device['name'] and device['address'] and device['connected'] is not None:
            ...
            bluetooth_devices.append({
                'type': 'file:skipcheck',
                'arg': device['address'],
                'subtitle': 'Connected' if is_connected else device['address'],
